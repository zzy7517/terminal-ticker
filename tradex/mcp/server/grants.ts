/** 管理外接编码 Runtime 每次运行所需的短期 MCP 授权。 */
import { createHash, randomBytes } from "node:crypto";
import type { ExternalAgentRuntimeId } from "../../agent/runtime/types.js";
...
export class McpRunGrantStore {
  ...
  /**
   * 为一次外接 Runtime run 签发随机 token，并只在内存中保存其 hash 和工具集合。
   * 工具集合来自 listToolsForExternalMcp：含 Message/Channel 协作写，永不含交易写。
   */
  issue(input: {
    tradexSessionId: string;
    ...
    this.grants.set(hashToken(token), {
      ...
      tools: input.registry.listToolsForExternalMcp(input.runtime ?? "claude-code"),
      expiresAt,
    });
