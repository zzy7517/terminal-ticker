"""Persist normalized OHLCV candles in a local SQLite cache."""
...
class CandleFetchPlan:
    """Describe how the provider should be queried after reading the cache."""
    after_open_time_ms: int | None
    limit: int


def default_cache_path() -> Path:
    """Return the default platform-local SQLite cache path."""
    base = Path(os.environ.get("XDG_CACHE_HOME", Path.home() / ".cache"))
    return base / DEFAULT_CACHE_SUBDIR / DEFAULT_CACHE_FILENAME


class CandleCache:
    """Store one-day normalized candle history per symbol and interval."""

    def __init__(self, path: str | Path, *, retention_seconds: int = 86_400) -> None:
        """Remember the SQLite path and retention window."""
        self.path = Path(path).expanduser()
        self.retention_seconds = retention_seconds

    @classmethod
    def from_config(cls, config: CacheConfig) -> "CandleCache":
        """Build a cache from app config."""
        return cls(
    ...
    def prune(self, *, now_ms: int | None = None) -> int:
        """Delete candles outside the retention window."""
        current_ms = _now_ms() if now_ms is None else now_ms
        cutoff_ms = current_ms - self.retention_seconds * 1000
    ...
    def upsert(
        ...
        *,
        ...
        """Insert or update normalized candles for a specific interval."""
        if not candles:
            return 0
    ...
    def latest_open_time_ms(
        ...
        *,
        ...
        """Return the newest retained candle open time for one symbol and interval."""
        current_ms = _now_ms() if now_ms is None else now_ms
        cutoff_ms = current_ms - self.retention_seconds * 1000
        with self._connect() as connection:
    ...
    def recent(
        ...
        *,
        ...
        """Return recent retained candles for one symbol and interval."""
        current_ms = _now_ms() if now_ms is None else now_ms
        cutoff_ms = current_ms - self.retention_seconds * 1000
        with self._connect() as connection:
...
def cached_fetch_candles(
    *,
    ...
    """Fetch candles through SQLite, using retained data to request only missing ranges."""
    current_ms = _now_ms() if now_ms is None else now_ms
    cache.prune(now_ms=current_ms)
    latest_open_ms = cache.latest_open_time_ms(symbol_key, interval, now_ms=current_ms)
    ...
    try:
        ...
        cached = cache.recent(symbol_key, interval, limit=limit, now_ms=current_ms)
        if cached:
            return cached
        raise
...
def _fetch_plan(
    ...
    *,
    ...
    """Choose the smallest safe provider fetch after reading the retained cache."""
